...
const setOriginEndpoint = (endpoint: string): string => {
    const originBaseUrl = config.api.originUrl;
    if (!originBaseUrl) {
        throw new Error('API_ORIGIN_URL is not configured. Set it via environment variable or runtime args.');
    }
    if (endpoint.includes('metrics')) {
        return `${originBaseUrl}/metrics/graphql`;
    }
    if (endpoint.includes('events')) {
        return `${originBaseUrl}/events/graphql`;
    }
    if (endpoint.includes('accounting')) {
        return `${originBaseUrl}/accounting/graphql`;
    }
    if (endpoint.includes('consumption')) {
        return `${originBaseUrl}/consumption/graphql`;
    }
    throw new Error('Unknown endpoint type');
}
